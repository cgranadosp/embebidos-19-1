...
///DEMONIO
	...
	pid_main = fork();   //Para hacer la creación del proceso
    if( pid_main == -1 )
    {
    ...
    if( pid_main )
    {
        //Se guarda en el archivo el id del proceso para utilizarlo después por el sistema de arranque
		syslog(LOG_INFO, "PID del segundo proceso hijo %d \n", pid_main);
		apArch = fopen("/run/demonio.pid", "w");
        apArch = fopen("/home/carlosg/demonio.pid", "w");
		fprintf(apArch, "%d", pid_main);
		fclose(apArch);
 ...
 */	
	...
	if(signal(SIGUSR1, ISRsw) == SIG_ERR){
		...
		syslog(LOG_INFO, "Error en la ISR\n");
		exit(EXIT_FAILURE);
	}
 ...
 */
   	...
   	if( (sockfd = socket (AF_INET, SOCK_STREAM, 0)) < 0 )
	{
		...
		syslog(LOG_INFO, "Ocurrió un problema en la creación del socket\n");
		exit(1);
   	}
/*
 ...
 */
   	...
   	if( bind(sockfd, (struct sockaddr *) &direccion_servidor, sizeof(direccion_servidor)) < 0 )
	{
		...
		syslog(LOG_INFO, "Ocurrió un problema al configurar el socket\n");
		exit(1);
   	}
/*
 ...
 */
	...
	if( listen(sockfd, COLA_CLIENTES) < 0 )
	{
		...
		syslog(LOG_INFO, "Ocurrió un problema al crear la cola de aceptar peticiones de los clientes\n");
		exit(1);
   	}
/*
 ...
 */
			...
			while(1)
			{
				...
				if(*valido==1){
					...
					*/
					...
					syslog(LOG_INFO, "Latitud grados: %s \n", lat_deg);
					syslog(LOG_INFO, "Latitud minutos: %s \n", lat_min);
					syslog(LOG_INFO, "Latitud direccion: %s \n", lat_dir);

					syslog(LOG_INFO, "Longitud grados: %s \n", lon_deg);
					syslog(LOG_INFO, "Longitud minutos: %s \n", lon_min);
					syslog(LOG_INFO, "Longitud direccion: %s \n", lon_dir);

					syslog(LOG_INFO, "Horas: %s \n", horas);
					syslog(LOG_INFO, "Minutos: %s \n", min);
					syslog(LOG_INFO, "Segundos: %s \n", seg);
				}
	...
	while(1){	
		...
		*/
			syslog(LOG_INFO, "Se aceptó un cliente, atendiéndolo \n");
			...
			syslog(LOG_INFO, "El cliente nos envió el siguiente mensaje: \n %s \n", leer_mensaje);
			...
			syslog(LOG_INFO, "Concluimos la ejecución de la aplicación Servidor \n");
		/*
...
void ISRsw(int sig){
	...
	if(sig == SIGUSR1){ //Si la señal que se recibe es SIGUSR1 entonces se trata del proceso hijo
		pid = wait(&estado);
		syslog(LOG_INFO, "USR1 recibida por el proceso %d \n", pid);
	}
}
